...
# 웹 서버에 요청을 보낼때 클라이언트의 정보를 함께 전달하여 실제 웹 브라우저인 것처럼 보이게 설정
headers = {
    'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36'
}

# 기사 본문 내용에서 의미 없는 내용을 제거 하기 위해 정규화를 활용하여 기자 이메일 주소 삭제
...
def get_news_list(keyword, toDate, fromDate) :
    ...
    for date in pd.date_range(toDate, fromDate) : #수집 날짜 설정
      ...
      while page <= 10:
            start = (page-1) * 10 + 1
            print(f"{str_d} 날짜의 {page} 페이지를 수집 중입니다.")
            ...
            if soup.select_one("div.not_found02"):
                print(f"{str_d} 날짜의 기사가 더 이상 없어 크롤링을 중단합니다.")
                break
